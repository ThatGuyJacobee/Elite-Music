...
export async function jellyfinAddTrack(
  ...
  const newTrack = new Track(player, {
    ...
    artist: (itemMetadata as any).AlbumArtist ?? (itemMetadata as any).Artists?.[0] ?? "Unknown Artist",
    url: streamUrl,
    thumbnail: thumbUrl,
...
export async function jellyfinAddPlaylist(
  ...
  for (const item of playlistEntries) {
    ...
    const newTrack = new Track(player, {
      ...
      artist: (item as any).AlbumArtist ?? (item as any).Artists?.[0] ?? "Unknown Artist",
      url: streamUrl,
      thumbnail: thumbUrl,
