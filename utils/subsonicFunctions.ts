  ...
  try {
    ...
    if (scope !== "track" && scope !== "album") {
      ...
      mappedPlaylists = playlistsMatch.map((playlistEntry) => ({
        ...
        duration:
          (playlistEntry as any).duration != null ? Number((playlistEntry as any).duration) * 1000 : 0,
      }));
    }
...
export async function subsonicAddPlaylist(
  ...
  const title =
    itemMetadata.title && itemMetadata.title !== ""
      ? itemMetadata.title
...
export async function subsonicAddAlbum(
  ...
  const albumArtist = (album.artist as string) || (album.albumArtist as string) || (itemMetadata.artist as string) || "";
