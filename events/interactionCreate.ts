...
export default {
  ...
  async execute(interaction: ButtonInteraction | StringSelectMenuInteraction | ChatInputCommandInteraction): Promise<void> {
      ...
      if (timestamp.has(interaction.user.id)) {
        ...
        if (curtime < expiration) {
          ...
          await interaction.reply({
            content: translate(interaction, "errors.cooldown", { time: timeleft, command: command.data.name }),
...
async function handleHelpSelectMenu(interaction: StringSelectMenuInteraction): Promise<void> {
  ...
  readdirSync("./commands/").forEach((dir) => {
    const commands = readdirSync(`./commands/${dir}`).filter((file) => file.endsWith(".js") || file.endsWith(".ts"));
    const cmds: any[] = [];
  ...
  if (value && value !== "home") {
    ...
    if (getchannel) {
      if (
        !(interaction.guild as Guild).members.me!.permissionsIn(interaction.channel!.id).has(PermissionFlagsBits.ViewChannel)
      ) {
...
async function handleButtonInteraction(interaction: ButtonInteraction): Promise<void> {
  ...
  const checkQueue = async () => {
    ...
    if (!queue || !queue.isPlaying()) {
      await interaction.reply({ content: translate(interaction, "queue.nothingPlaying"), flags: MessageFlags.Ephemeral });
      return null;
    }
    return queue;
  };
  ...
  if (customId === "np-pauseresume") {
    ...
    try {
      ...
      await interaction.reply(ephemeralReply({
        content: translate(interaction, "errors.failedToPauseResume", { action: checkPause ? "resuming" : "pausing" }),
      }));
    }
  }
