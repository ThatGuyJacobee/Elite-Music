...
async function runSubsonicFlow(
    ...
    try {
                        ...
                        new StringSelectMenuOptionBuilder()
                            .setLabel(
                                (item.title as string).length > 100
                                    ? `${(item.title as string).substring(0, 97)}...`
            ...
            if (results.albums) {
                for (const item of results.albums) {
                    ...
                    const albumTitle = (item as any).artist
                        ? `${item.title as string} - ${(item as any).artist as string}`
                        : (item.title as string);
                    embedFields.push({
        ...
        if (itemFound.type == "playlist") {
            await subsonicAddPlaylist(interaction, itemFound, "send", playlistOrder, playNextFlag);
        } else if (itemFound.type == "album") {
            await subsonicAddAlbum(interaction, itemFound, "send", playlistOrder, playNextFlag);
        } else {
            await subsonicAddTrack(interaction, playNextFlag, itemFound, "send");
        }
    } catch (err) {
        console.log(err);
