...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!queuedTracks[removeamount - 1])
            return interaction.reply({
                content: `❌ | There is no song at the queried position in the queue. Please try again.`,
                ephemeral: true,
            });
        ...
        const removeembed = new EmbedBuilder()
            ...
            .setDescription(
                `Removed track ${queuedTracks[removeamount - 1].title} ${queuedTracks[removeamount - 1].queryType != "arbitrary" ? `([Link](${queuedTracks[removeamount - 1].url}))` : ""} from the queue!`,
            )
            .setTimestamp()
            .setFooter({
        ...
        try {
            ...
            interaction.reply({
                content: `❌ | Ooops... something went wrong, there was an error removing the song from the queue. Please try again.`,
                ephemeral: true,
            });
        }
    },
