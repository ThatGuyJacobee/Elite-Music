        ...
        if (shouldShowPicker) {
            ...
            if (searchResults.songs) {
                for (const song of searchResults.songs) {
                    ...
                    (actionRowSelect.components[0] as StringSelectMenuBuilder).addOptions(
                        ...
                        new StringSelectMenuOptionBuilder()
                            .setLabel(
                                (playlist.title as string).length > 100
                                    ? `${(playlist.title as string).substring(0, 97)}...`
            ...
            if (searchResults.albums) {
                for (const album of searchResults.albums) {
                    ...
                    const albumTitle = (album as any).parentTitle
                        ? `${album.title as string} - ${(album as any).parentTitle as string}`
...
client.on("interactionCreate", async (interaction: any) => {
    ...
    for await (const selectedValue of interaction.values) {
        ...
        const playlistOrder =
            orderSegment != null && orderSegment.startsWith("order=") ? orderSegment.split("order=")[1] : "sequential";
        ...
        const metadataJson = (await metadataRequest.json()) as Record<string, unknown>;
        ...
        if (itemType == "playlist") {
            ...
            await plexAddPlaylist(
                interaction,
                metadataJson.MediaContainer as Record<string, unknown>,
                "edit",
                playlistOrder,
                usePlayNext,
            );
            ...
            (metadataJson.MediaContainer as any).type = itemType;
            const mediaContainer = metadataJson.MediaContainer as Record<string, unknown>;
            await plexAddAlbum(
                interaction,
                (mediaContainer.Metadata as Record<string, unknown>)[0] as Record<string, unknown>,
                "edit",
                playlistOrder,
                usePlayNext,
            );
        } else {
            const mediaContainer2 = metadataJson.MediaContainer as Record<string, unknown>;
            await plexAddTrack(
                interaction,
                usePlayNext,
                (mediaContainer2.Metadata as Record<string, unknown>)[0] as Record<string, unknown>,
                "edit",
            );
        }
    }
