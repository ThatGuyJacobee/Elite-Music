...
module.exports = {
    ...
    async execute(interaction) {
        ...
        for (i; i < curPage * 10; i++) {
            if (previousTracks[i]) {
                curTracks.push({
                    ...
                    value: `**${previousTracks[i].author}** ${previousTracks[i].queryType != "arbitrary" ? `([Link](${previousTracks[i].url}))` : ""}`,
                });
            }
        }
        ...
        var finalComponents = new ActionRowBuilder().addComponents(
            ...
            //.addOptions(options)
            new ButtonBuilder().setCustomId(`queuehistory-${timestamp}-previous`).setStyle(1).setLabel("⬅️"),
            new ButtonBuilder().setCustomId(`queuehistory-${timestamp}-next`).setStyle(1).setLabel("➡️"),
        );
        ...
        collector.on("collect", async (buttonResponse) => {
            ...
            queueembed.setFooter({
                text: `Requested by: ${interaction.user.discriminator != 0 ? interaction.user.tag : interaction.user.username} - Page ${curPage}`,
            });
        ...
        collector.on("end", async () => {
            interaction.editReply({
                content: "Please use **/queuehistory** again to show the embed again.",
                components: [],
            });
        });
    },
