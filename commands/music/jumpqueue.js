...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const jumpembed = new EmbedBuilder()
            ...
            .setDescription(
                `Now playing: ${queuedTracks[trackIndex].title} ${queuedTracks[trackIndex].queryType != "arbitrary" ? `([Link](${queuedTracks[trackIndex].url}))` : ""}!`,
            )
            .setTimestamp()
            .setFooter({
        ...
        try {
            ...
            console.log(err);
            interaction.reply({
                content: `❌ | Ooops... something went wrong, there was an error jumping queue. Please try again.`,
                ephemeral: true,
            });
        }
    },
